#ifndef SRC_LIBS_GRAPH_HPP_
#define SRC_LIBS_GRAPH_HPP_
...
template <typename T>
Graph<T>::Graph(std::size_t elements_count, bool is_directed)
...
template <typename T>
Graph<T>::Graph(std::size_t elements_count, bool is_directed, const T& weight)
: graph_(std::vector<std::vector<T>>(elements_count)), is_directed(is_directed) {
    for (std::size_t i = 0; i < elements_count; ++i) {
        this->graph_[i] = std::vector<T>(elements_count);
        for (std::size_t j = 0; j < elements_count; ++j) {
...
template <typename T>
void Graph<T>::new_edge(std::size_t first_element, std::size_t second_element, const T& weight) {
...
template <typename T>
void Graph<T>::delete_edge(std::size_t first_element, std::size_t second_element) {
...
template <typename T>
void Graph<T>::delete_vertex(std::size_t element) {
    for (std::size_t i = 0; i < this->graph_.size(); ++i) {
...
template <typename T>
const T& Graph<T>::get_weight(std::size_t first_element, std::size_t second_element) {
    return this->graph_[first_element][second_element];
}

template <typename T>
std::size_t Graph<T>::size() { return this->graph_.size(); }

template <typename T>
std::vector<T> Graph<T>::find_shortest_path(std::size_t start_element) {
...
template <typename T>
std::vector<T> Graph<T>::dijkstra(std::size_t start_element, std::size_t end_element, bool &error_message) {
...
template <typename T>
std::vector<T> Graph<T>::bellman_ford(std::size_t start_point, bool &error_message) {
    error_message = 0;
...
template <typename T>
Graph<std::size_t> Graph<T>::floyd_wallsher() {
...
template <typename T>
void Graph<T>::topological_sort_rec(std::size_t point, std::vector<bool> &visited, std::stack<std::size_t> &stack) {
    visited[point] = true;
...
template <typename T>
std::vector<std::size_t> Graph<T>::topological_sort() {
...
template <typename T>
void Graph<T>::depth_first_search(std::size_t start, std::vector<bool> &is_used) {
    is_used[start] = true;
...
template <typename T>
std::vector<std::vector<bool>> Graph<T>::find_connected_components() {
...
template <typename T>
std::vector<T> &Graph<T>::operator [](std::size_t idx) {
    return this->graph_[idx];
}

#endif  // SRC_LIBS_GRAPH_HPP_
